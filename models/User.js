...
const TAX_RATES = {
  ...
  provincial: {
    ...
    // HST provinces: only provincial portion here
    NB: { type: 'HST', rate: 0.10 },
    NL: { type: 'HST', rate: 0.10 },
    NS: { type: 'HST', rate: 0.10 },
    ON: { type: 'HST', rate: 0.08 },
    PE: { type: 'HST', rate: 0.10 },
...
const userSchema = new mongoose.Schema(
  {
    ...
    businessNumber: {
      ...
      validate: {
        validator: function (v) {
          if (!v) return true;
          const regex = this.province === 'QC' ? /^\d{9}RQ\d{4}$/ : /^\d{9}RT\d{4}$/;
          return regex.test(v);
        },
...
userSchema.virtual('businessNumberFormat').get(function () {
  const format = this.province === 'QC' ? BUSINESS_NUMBER_FORMAT.QC : BUSINESS_NUMBER_FORMAT.default;
